...
printf "/n...Selecting only sites called in 80% (11+) of samples\n" | tee >(cat >&2)
gatk --java-options "-Xmx${MEM}g" SelectVariants \
	-V ${WORKING_DIR}/VCFs/${BATCH_NAME}.pass.allsites.fs.vcf.gz \
	--max-nocall-number 2 \
	--exclude-filtered \
	-O $WORKING_DIR/VCFs/${BATCH_NAME}.pass.11called.vcf.gz
...
printf "/n...Selecting only SNPs called in 80% (11+) of samples\n" | tee >(cat >&2)
gatk --java-options "-Xmx${MEM}g" SelectVariants \
	-V ${WORKING_DIR}/VCFs/${BATCH_NAME}.pass.SNPs.fs.vcf.gz \
	--max-nocall-number 2 \
	--exclude-filtered \
	-O $WORKING_DIR/VCFs/${BATCH_NAME}.pass.11calledSNPs.vcf.gz
