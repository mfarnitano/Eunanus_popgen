...
FASTQ_TABLE=${SCRIPTS_DIR}/inputs/Brevipes_complete_fastq_table.txt #three columns, directory, prefix, seqbatch
...
while read -r line; do
	...
	SEQ_BATCH=$(echo -n $line | tr -s ' ' | cut -d' ' -f3)
	printf "\n...submitting sample: $INPUT_PREFIX\n" | tee >(cat >&2)
	sbatch ${SCRIPTS_DIR}/genotype_each.sh $SCRIPTS_DIR $BATCH_NAME $GENOME $INPUT_DIR $INPUT_PREFIX $SEQ_BATCH
	printf "%s\t%s%s\n" $INPUT_PREFIX $INPUT_PREFIX ".ar.g.vcf.gz" >> ${WORKING_DIR}/gvcf_map.txt
done < $FASTQ_TABLE
